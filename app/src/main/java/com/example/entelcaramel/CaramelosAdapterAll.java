...
public class CaramelosAdapterAll extends RecyclerView.Adapter<CaramelosViewHolder> {

    private ArrayList<ArrayList> caramelos;

    CaramelosAdapterAll(ArrayList<ArrayList> caramelos){
    ...
    public void onBindViewHolder(@NonNull CaramelosViewHolder holder, int i) {
            ...
            for(int k =0;k<caramelos.size();k++){
                for(int l = 0;l<caramelos.get(k).size();l++){
                    Caramelos cara = (Caramelos) caramelos.get(k).get(l);
                    switch (cara.getEnvoltorio()) {
                        case "Rojo":
